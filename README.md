...
- **Lessons learned:** The resources I used to build this site are above. I learned a valuable lesson about how `less is more` and how the `download attribute`to save a `<canvas>` as a PNG works `<a href="" download="my_painting.png">Download my painting</a>` which was not used in my project. I used a differrent method through trial and error and relying on AI to help me, because I made the mistake by placing the  `<a href="" download="my_painting.png">Download my painting</a>` further below my nav links which somehow interfered with the download, because I wanted the link to be located just above the canvas.

- So, I changed my js to target the first `<a>` without using `download=` in `<a>`; however, I did test `<a href="" download="my_painting.png">Download my painting</a>` and it works perfectly if I moved it above my nav without having to write canvas.toBlob etc. Basically, my code has unnecessary "extra lines". My version is 128 lines versus a much cleaner version of 98 lines. I put the cleaner version below displaying the download logic which is much easier to understand. We learn from our mistakes.
